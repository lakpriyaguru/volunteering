...
if (isset($_POST['login'])) {
    ...
    $query = mysqli_query($con, "SELECT adminID, adminName, adminType FROM admin WHERE adminEmail='$email' AND adminPassword='$password'");
    $ret = mysqli_fetch_array($query);
    if ($ret > 0) {
        $_SESSION['adminID'] = $ret['adminID'];
        $_SESSION['adminName'] = $ret['adminName'];
        $_SESSION['adminType'] = $ret['adminType'];

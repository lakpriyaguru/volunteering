...
if (strlen($_SESSION['adminID']) == 0) {
  ...
  if (isset($_POST['change'])) {
    ...
    $query = mysqli_query($con, "select adminID from admin where adminID='$adminID' and adminPassword='$password'");
    $row = mysqli_fetch_array($query);
    if ($row > 0) {
      $ret = mysqli_query($con, "update admin set adminPassword='$newpassword' where adminID='$adminID'");

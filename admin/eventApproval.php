...
if (strlen($_SESSION['adminID']) == 0) {
    ...
    if ($_GET['action'] == 'approve') {
        ...
        $query = mysqli_query($con, "update event set eventApproval = 'Approved' where eventID = '$eventID'");
        if ($query) {
            echo "<script>
                    Swal.fire({
                        icon: 'success',
    ...
    if ($_GET['action'] == 'decline') {
        $eventID = intval($_GET['said']);
        $query = mysqli_query($con, "update event set eventApproval = 'Declined' where eventID = '$eventID'");
        if ($query) {
            echo "<script>
                    Swal.fire({
                        icon: 'success',
